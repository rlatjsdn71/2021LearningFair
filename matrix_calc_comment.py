...
def subtraction_p(a, b):
    ...
    for i in range(len(a)):
        for j in range((len(a[0]))):
           sub[i][j] = a[i][j] - b[i][j] # suB에 더한 값을 저장
    return sub # 반환
...
def multiple_p(a, b) :
    if type(a) == int or type(a) == float: #mat1이 숫자라면,
        res = [len(b[0])*[0]for i in range(len(b))] # matR을 선언하고
        for i in range(len(res)): #각 행의
            for j in range(len(res[0])): #각 열의 값에
                res[i][j] = b[i][j] * a # mat1을 곱한 값을 matR[i][j]에 저장한다.
    else: #mat1이 정수가 아니라면,
        res = [len(b[0])*[0] for i in range(len(a))] # matR을 선언하고
